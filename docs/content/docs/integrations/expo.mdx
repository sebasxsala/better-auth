...
The Ruby port supports the server-side Expo integration through `BetterAuth::Plugins.expo`.
The native Expo client remains the upstream JavaScript package, because SecureStore,
browser sessions, focus/online managers, and React Native fetch hooks run inside the
Expo app rather than the Ruby server.
...
The plugin adds `/expo-authorization-proxy`, copies `expo-origin` into `Origin`
when the regular origin is missing, and injects auth cookies into trusted
non-HTTP deep-link redirects. In development, it also trusts `exp://`.

## Expo Client

Use the upstream Expo client package inside the Expo app:
...
Native cookie storage, browser-opening flow, `getCookie`, focus/online managers,
ID-token client header behavior, and `lastLoginMethodClient` are intentionally
handled by `@better-auth/expo` rather than by the Ruby gem.

{/*
Hidden upstream content for Expo Integration.
