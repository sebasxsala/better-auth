...
* The provider is not listed in `account.account_linking.trusted_providers`.
...
* Add the provider id (e.g., `github`, `google`) to `account.account_linking.trusted_providers` in your auth config.
...
* Double-check that the runtime sees the intended `trusted_providers` list.
