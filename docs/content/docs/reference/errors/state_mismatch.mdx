...
3. **Secondary storage (Redis / KV) without database fallback.** When `secondary_storage` is configured,
   ...
   (TTL expiry, memory pressure, server restart) and `verification.store_in_database` is not explicitly `true`,
...
* If using `secondary_storage`, either set `verification.store_in_database: true` as a fallback, or ensure
...
When `store_state_strategy` is `"cookie"`, all state data is encrypted into a cookie. This error means the
...
* The signed cookie expired (its `max_age` is 5 minutes, shorter than the 10-minute DB record expiry).
...
* For SAML flows, Better Auth already sets `skip_state_cookie_check` internally.
...
| **Medium**    | Secondary storage (Redis) key evicted without DB fallback             | `state_mismatch` (DB)                                           | Set `verification.store_in_database: true` or ensure Redis persistence                  |
