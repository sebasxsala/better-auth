...
`after_verification` may return `{ user_id: "..." }` during passkey-first registration to link the verified passkey to a concrete user. During session-required registration, returning a different user ID is rejected.
