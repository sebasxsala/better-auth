...
{/*
Hidden upstream content for Facebook.
...
For limited login, you need to pass `idToken.token`, for only `accessToken` you need to pass `idToken.accessToken` and `idToken.token` together because of (#1183)[https://github.com/sebasxsala/better-auth/issues/1183].
