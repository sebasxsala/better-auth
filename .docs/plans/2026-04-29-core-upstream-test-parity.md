...
- [x] Translate all server-applicable upstream test titles into Ruby Minitest tests or document a Ruby-specific exclusion. OAuth/social is complete in child plan `2026-04-30-core-oauth-social-upstream-parity.md`; auth/context/call parity is complete in child plan `2026-04-30-core-auth-context-upstream-parity.md`.
...
| `upstream/packages/better-auth/src/context/{create-context,init,init-minimal}.test.ts`, `upstream/packages/better-auth/src/auth/{full,minimal,trusted-origins}.test.ts`, `upstream/packages/better-auth/src/call.test.ts` | Auth construction, per-request context, dynamic baseURL, trusted origins, direct API/Rack call semantics | `packages/better_auth/test/better_auth/auth_context_upstream_parity_test.rb`, `packages/better_auth/test/better_auth/configuration_test.rb`, `packages/better_auth/test/better_auth/{api,router,plugin,auth}_test.rb` | Ported | Child plan `2026-04-30-core-auth-context-upstream-parity.md` implemented Ruby-applicable server behavior. Kysely adapter/migration internals and browser client `$fetch` semantics are documented Ruby exclusions. |

## Partial Suite Follow-Up

- [x] Audit and implement `context/create-context.test.ts`, `auth/full.test.ts`, `auth/minimal.test.ts`, `auth/trusted-origins.test.ts`, and `call.test.ts` against Ruby core tests. Child plan: `2026-04-30-core-auth-context-upstream-parity.md`.
