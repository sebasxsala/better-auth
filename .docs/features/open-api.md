...
- Represents upstream Zod-derived request/response behavior through the generated Ruby schema contract for selected server-relevant routes, including nested `idToken`, OpenAPI 3.1 nullable arrays, defaulted `rememberMe`, boolean optional fields, nullable `/get-session` responses, and richer `/change-email` plus `/change-password` schemas.
...
2026-05-04 progress: upstream `open-api.test.ts.snap` includes 30 base paths. Ruby now has richer upstream-style OpenAPI schemas for 6 of them (`/sign-in/email`, `/sign-in/social`, `/sign-up/email`, `/get-session`, `/change-email`, `/change-password`) plus global document metadata, model metadata, security schemes, default errors, path-param conversion, disabled paths, and reference-page options. Roughly 24 base paths still need rich request/response schemas before exact snapshot parity is plausible.
