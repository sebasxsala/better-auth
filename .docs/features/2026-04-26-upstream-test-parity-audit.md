...
Update 2026-04-28: Ruby OpenAPI now covers OpenAPI 3.1.1 document metadata, security schemes, servers, model field defaults/readOnly/date formats, `:param` path conversion, disabled paths, selected email/social request bodies, `/change-email`, `/change-password`, default errors, reference HTML, theme, nonce, and disable-reference behavior. It still remains Partial for exact upstream parity because the upstream snapshot has 30 base paths and roughly 25 still need rich request/response schemas plus exact Scalar HTML/configuration.
