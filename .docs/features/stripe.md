...
- Supports synchronous webhook construction and async-style injected webhook clients when provided by the application wrapper.
...
- Webhook verification delegates to the injected client through `webhooks.construct_event`; wrappers can expose `construct_event_async` for async-style parity.
...
- Plan lookup, annual price IDs, seat quantity changes, duplicate active subscription rejection, and one-trial-per-reference protection across plans.
- Customer de-duplication by Stripe search, custom customer-create params, user email sync, organization customer creation, organization name sync, and active-subscription deletion guard.
