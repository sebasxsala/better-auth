...
Ports the separate upstream OAuth provider package into the Ruby core plugin namespace for metadata, client registration/lookup, client credentials tokens, introspection, revocation, and userinfo support.

## Ruby Adaptation

- Exposed as `BetterAuth::Plugins.oauth_provider`.
- Adds `/.well-known/oauth-authorization-server`, `/.well-known/openid-configuration`, `/oauth2/register`, `/oauth2/client`, `/oauth2/client/:id`, `/oauth2/clients`, `/oauth2/token`, `/oauth2/introspect`, `/oauth2/revoke`, and `/oauth2/userinfo`.
...
- The Ruby port currently implements the core machine-client and metadata surface. Authorization-code, organization reference, consent-management, logout, and deeper rate-limit matrices remain future polish.
