...
| SSO | [x] Supported | Provider CRUD/access, OIDC discovery/callback, SAML ACS/callback/metadata, RelayState safety, replay protection, XML assertion/algorithm validation decisions, domain verification, and organization assignment are implemented. SAML cryptographic verification/decryption is delegated to `validate_response`. |
