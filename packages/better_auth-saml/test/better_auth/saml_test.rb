...
class BetterAuthSAMLTest < Minitest::Test
  SECRET = "better-auth-saml-test-secret-with-entropy"
  ...
  def setup
    skip "ruby-saml is not installed" unless defined?(BetterAuth::SAML) && defined?(OneLogin::RubySaml)
  ...
  def test_sso_options_exposes_core_parse_response_hook
    options = BetterAuth::SAML.sso_options
  ...
  def test_encrypted_assertion_decryption_settings_are_wired_to_ruby_saml
    ...
    settings = BetterAuth::SAML.build_settings(
      provider,
      ctx,
  ...
  def build_auth
    BetterAuth.auth(
      ...
      secret: SECRET,
      database: :memory,
      plugins: [BetterAuth::Plugins.sso(BetterAuth::SAML.sso_options)]
    )
  end
